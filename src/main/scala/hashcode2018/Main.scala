...
case class Ride(from: (Int, Int), to: (Int, Int), minStart: Int, maxEnd: Int)
...
object Main extends App {
  ...
  def readInput(inputName: String): Input = {
    ...
    val rides = rideLines.map(_.split(" ").toList.map(_.toInt)).map {
      case fromX :: fromY :: toX :: toY :: minStart :: maxEnd :: Nil =>
        Ride((fromX, fromY), (toX, toY), minStart, maxEnd)
    }.toVector
